...
A reactive component is a set of state variables and a set of transactions that operate on those state variables.
Linking a transaction in one component to a transaction in another component yields another transaction that operates on the state variables of the constituent components.
A transaction-oriented approach resolves the main difficulties of the control-oriented imperative approach.
In the reactive component model, transactions are atomic.
This relieves developers from the burden of identifying and guarding critical sections.
The composability of transactions allows developers to create complex state transitions independent of control flow.
...
In Chapter~\ref{implementation}, we describe the implementation of an interpreter for \rcgo including the algorithm that checks a system for sound composition, a calling convention for transactions, the implementation of move semantics, and an approach to file descriptor I/O.
