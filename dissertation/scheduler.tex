...
For example, a scheduler that processes transactions in first-in first-out (FIFO) order eventually selects and executes every transaction in the queue.
...
When one of the transactions terminates, the scheduler must select another transaction since it is work conserving.
For safety, the scheduler will select another transaction from $S$ as it is unsafe to select and execute $\tau$.
This pattern of behavior induced by the work conserving nature of the scheduler may perpetually deny service to $\tau$.
For fairness, the scheduler must refrain from selecting another transaction to execute so that the $\tau$ transaction can be serviced.
